...
import { optionBDayFile } from "../../../../../../lib/dvc/experimentalPaths";

export default function handler(req: NextApiRequest, res: NextApiResponse) {
  const monthSlug = String(req.query.month ?? "");
  const daySlug = String(req.query.day ?? "").replace(/^0+/, "") || "0";
  ...
  const dayNum = parseInt(daySlug, 10);
  if (!dayNum || dayNum < 1 || dayNum > config.days) {
    res.status(404).json({ error: "Day not found" });
    return;
  }

  const filePath = optionBDayFile(config.slug, String(dayNum));
